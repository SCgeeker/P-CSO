...
**包含**：組織筆記 → 生成草稿 → 句法優化 → 連貫性與語域優化 → 術語與主張稽核 → 診斷報告
**適用**：需要系統化優化的手稿準備
**時間**：2-4 小時（完整章節）

#### 2. `pinker-syntax` - 句法優化
**用途**：應用 11 條句法規則優化句子結構（第 4 章 6 條認知規則，加 5 條子句與連接詞規範）
**解決**：複雜句法、冗長表述、歧義結構、語法不一致
**適用**：句子難以理解、中心嵌入、左分支結構過多的文本
**時間**：30 分鐘

#### 3. `pinker-coherence` - 連貫性優化
**用途**：應用 8 條原則優化段落與多句流暢度、修正學術語域
**解決**：主題模糊、過渡不清、實體追蹤混亂、邏輯隱含、主語施事錯置、稿件讀起來像研究筆記
**適用**：段落流暢性弱、過渡不自然的文本
**時間**：30 分鐘
...
| 7-10. 子句規範 | em-dash、括號列表、關係子句、從屬子句 | 拆為獨立句或改用 SVO | "X. Y." 而非 "X, which Y" |
| 11. 連接詞負荷 | 一句多個 and；以 and 表對比 | 拆句並換用精確連接詞 | "but"、"because" 而非 "and" |
...
| 7. 元話語與語域 | 以作者為中心的文本；讀起來像研究筆記 | 刪除 "I will discuss" 短語；刪除只標示論證而不推進論證的句子 | 直接討論；段落開頭放主張而非交叉引用 |
| 8. 主語施事 | 抽象名詞佔據主語位置 | 該點名的人點名，該陳述的主張陳述 | "The study found" → "作者發現" |
