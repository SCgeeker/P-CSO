...
description: Review and refactor scientific manuscript prose for clarity, eliminate clutter, and ensure keyword consistency. Triggers: review my writing, check my manuscript, improve clarity, clean up prose, edit for journal submission, fix passive voice.
---
...
| **section-review** | "review the Introduction," "check the Discussion" | Run all six passes on a single section |
...
**Field-register test for content words:**

Consistency does not settle whether a term belongs to the readership's vocabulary at
all. Check each load-bearing content word against usage in the target field, not
against general English. Real cases:

| Term | Problem | Replacement |
|---|---|---|
| `licenses` (a cue licenses a response) | Reads as syntactic theory, not memory research | `elicits` |
| `record` (as a noun for a body of findings) | Not how this readership uses the word | `findings`, or name them |
| `wait on` (for "depend on") | Unidiomatic for this readership | `depend on` |
| `stage N2b` (internal scoring code) | Jargon carrying no meaning for the reader | State the fact without the code |
| `attention displays` | Covers eye-tracking but not similarity judgment | `attention studies` |
| `a listener` | Names a role the studies did not test | `native speakers` |

**When a term is challenged, count before ruling:**

1. Grep the manuscript and count occurrences and positions. A term appearing five
   times, including in a section heading, is the manuscript's own vocabulary. Keep it
   and let paragraphs land on it.
2. If the term came from a cited work, mark it as theirs on first use instead of
   adopting it as neutral: "They group the two studies under one category, consolidation
   evidence, and that category does not separate the stages."
3. If the term is replaced, replace **every** occurrence in one pass, then list the
   occurrences deliberately kept and why. Replacing it in one section while three
   others keep it is worse than not replacing it at all.

**A term must not presuppose what the manuscript denies.**

"The consolidation benefit of each nap" presupposes that every nap produces a benefit,
which is precisely what the manuscript argues against for naps below a duration
threshold. Wording that smuggles in a denied premise is a content error, not a style
preference: replace it with wording that states the contrast the argument actually
makes — "whether a nap reaches slow-wave sleep or ends before it." See Pass 6.
...
### Pass 6: Claim-Evidence Discipline

Passes 1 through 5 improve delivery. This pass checks that the prose does not commit
the manuscript to more than its evidence supports. It is the one pass that touches
substance, so it **flags and proposes** rather than silently rewrites.

**Unsupported concession.** A concession offered before an argument hands the reviewer
a lever. "This method potentially isolates the naps with and without slow-wave sleep"
conceded a separation the manuscript has no data for, in the opening of a paragraph
arguing that the method cannot separate them. Cut it, or mark it explicitly as a
hypothetical that the following sentences then close.

**Hedged accusation.** "The researchers' lack of sleep-stage monitoring possibly
degraded the validity of their findings" reads as an accusation, and the hedge does
not soften it. Two sentences later, the same paragraph said "The consequence is not
that a published effect is wrong." A reviewer holds the manuscript to the stronger
reading and then catches the retraction. If the position is undecidability, state
undecidability.

**Intra-paragraph contradiction.** Read each paragraph for a sentence that a later
sentence withdraws. This is the most common way a hedged claim gets caught in review.

**Checklist:**

- [ ] Every concession is one the evidence supports, or is marked hypothetical and closed
- [ ] No sentence attributes a fault to cited authors that the manuscript elsewhere declines to assert
- [ ] No paragraph contains a claim that a later sentence in the same paragraph withdraws
- [ ] No terminology presupposes a premise the manuscript argues against (Pass 4)
- [ ] Every number and attribution in the prose was checked against the source rather than inferred (Pass 5)
...
### Pass 6: Claim-Evidence Discipline — [N issues found]
[Same format. Flag only; propose the revision and let the author decide.]
...
2. Show a revised version with all six passes applied
...
- **Never alter scientific content.** You improve delivery, not substance. If a
  claim seems wrong, flag it as a content note but do not change it. Pass 6 is the
  one place where content is examined, and it reports rather than edits.
